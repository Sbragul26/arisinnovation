...
const AboutPage = ({ scrollToSection }) => {
  ...
  const values = [
    {
      icon: <Target className="w-12 h-12 text-[#FFFFFF]" />,
      title: 'Purpose-Driven',
      description: 'Crafting solutions that align with your vision for meaningful impact.',
    },
    {
      icon: <Lightbulb className="w-12 h-12 text-[#FFFFFF]" />,
  ...
  const stats = [
    { 
      ...
      gradient: 'from-[#241A7A] to-[#1A135A]',
      shadowColor: 'shadow-[#241A7A]/25'
    },
    { 
      icon: <Users className="w-10 h-10 text-[#FFFFFF]" />, 
      value: '25+', 
      label: 'Happy Clients', 
      gradient: 'from-[#1A135A] to-[#241A7A]',
      shadowColor: 'shadow-[#1A135A]/25'
    },
    { 
  ...
  return (
    ...
    >
        ...
        {[...Array(8)].map((_, i) => (
          <motion.div
            ...
            style={{
              top: `${10 + (i * 12)}%`,
              left: `${5 + (i * 11)}%`,
              background: i % 3 === 0 ? '#241A7A/30' : i % 3 === 1 ? '#1A135A/30' : '#FFFFFF/30'
            }}
      ...
      {/* Decorative elements */}
      <motion.div 
        className="absolute top-20 right-20 w-32 h-32 bg-[#241A7A]/40 rounded-full blur-3xl"
        variants={floatingVariants}
        animate="float"
      />
      <motion.div 
        className="absolute bottom-32 left-16 w-40 h-40 bg-[#1A135A]/40 rounded-full blur-3xl"
        variants={floatingVariants}
        animate="float"
        transition={{ delay: 1 }}
      />
      <motion.div 
        className="absolute top-1/2 right-1/3 w-24 h-24 bg-[#FFFFFF]/30 rounded-full blur-2xl"
        variants={floatingVariants}
        animate="float"
        transition={{ delay: 2 }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto text-center">
        ...
        >
          ...
          <span className="text-[#FFFFFF] font-semibold">precision</span> and{' '}
          <span className="text-[#FFFFFF] font-semibold">innovation</span>
        </motion.p>
        ...
        >
          <motion.div 
            className="relative bg-[#241A7A]/80 backdrop-blur-xl rounded-2xl p-6 md:p-10 border border-[#1A135A]/30 shadow-2xl"
          ...
          >
            <motion.h2 
              className="text-3xl md:text-4xl font-black mb-6 text-[#FFFFFF]"
              variants={itemVariants}
            >
              Our Story
            </motion.h2>
            ...
            <motion.p 
              className="text-lg md:text-xl text-[#FFFFFF] leading-relaxed mb-6 font-medium"
              variants={itemVariants}
            >
            ...
            <motion.p 
              className="text-base md:text-lg text-[#FFFFFF] leading-relaxed mb-8"
              variants={itemVariants}
            >
            ...
            <motion.div
              className="bg-gradient-to-r from-[#241A7A]/10 to-[#1A135A]/10 rounded-xl p-6 border border-[#1A135A]/20"
              variants={itemVariants}
            >
              <h3 className="text-xl md:text-2xl font-bold mb-4 text-[#FFFFFF]">
                Mission Statement
              </h3>
              <p className="text-lg md:text-xl text-[#FFFFFF] leading-relaxed italic font-medium">
        ...
        >
          {stats.map((stat, index) => (
            ...
            >
              <motion.div
                className={`relative flex flex-col items-center justify-center h-full p-6 rounded-2xl bg-[#241A7A]/90 backdrop-blur-xl border border-[#1A135A]/40 shadow-2xl ${stat.shadowColor} hover:shadow-2xl transition-all duration-500`}
                variants={cardHoverVariants}
                whileHover="hover"
                whileTap="tap"
              >
                ...
                <motion.div 
                  className="text-3xl md:text-4xl font-black text-[#FFFFFF] mb-3"
        ...
        >
          <motion.h2 
            className="text-3xl md:text-4xl font-black mb-12 text-[#FFFFFF]"
            variants={itemVariants}
          >
            Core Values
          </motion.h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {values.map((value, index) => (
              ...
              >
                <motion.div
                  className="relative bg-[#241A7A]/90 backdrop-blur-xl rounded-2xl p-6 border border-[#1A135A]/40 shadow-2xl hover:shadow-[#241A7A]/10 transition-all duration-500 h-full flex flex-col items-center justify-center text-center"
                  variants={cardHoverVariants}
                  whileHover="hover"
                  whileTap="tap"
                >
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {differentiators.map((item, index) => (
              ...
              >
                <motion.div
                  className={`relative flex flex-col items-center justify-center p-6 rounded-2xl bg-[#241A7A]/90 backdrop-blur-xl border border-[#1A135A]/40 shadow-2xl hover:shadow-2xl transition-all duration-500 h-full text-center`}
                  variants={cardHoverVariants}
                  whileHover="hover"
                  whileTap="tap"
                  style={{
                    background: `linear-gradient(135deg, #241A7A/90, #1A135A/90), ${item.gradient}`
                  }}
                >
                  <motion.div 
                  ...
                  <p className="text-[#FFFFFF] leading-relaxed text-sm">{item.description}</p>
                </motion.div>
              </motion.div>
            ))}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {teamMembers.map((member, index) => (
              ...
              >
                <motion.div
                  className="relative bg-[#241A7A]/90 backdrop-blur-xl rounded-2xl p-6 text-center border border-[#1A135A]/40 shadow-2xl hover:shadow-[#241A7A]/10 transition-all duration-500 h-full flex flex-col items-center justify-center"
                  variants={cardHoverVariants}
                  whileHover="hover"
                  whileTap="tap"
                >
